...
const ReviewPanel = ({ application, onClose, onStatusChange }) => {
    ...
    const hasReceipt = application.documents?.some(doc => doc.document_type === 'fee_receipt');
                        ...
                        <div className="flex gap-1">
                            {application.documents?.map((_, idx) => (
                                <button
                                    ...
                                    className={`w-1.5 h-1.5 rounded-full transition-colors ${idx === activeDocIndex ? 'bg-emerald-600' : 'bg-slate-300 dark:bg-slate-600'}`}
                                />
                            ))}
                        </div>
                    </div>
            ...
            <div className="w-[450px] flex flex-col bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-700">
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8">
                    <section>
                        ...
                        <div className="grid grid-cols-2 gap-y-6 gap-x-4">
                            ...
                            <InfoItem label="Sexe" value={application.profile?.gender === 'Male' ? 'Homme' : (application.profile?.gender === 'Female' ? 'Femme' : 'Non précisé')} />
                            <InfoItem label="Ville / Province" value={`${application.profile?.city || ''} / ${application.profile?.province || ''}`} />
                            <InfoItem label="CIN" value={application.profile?.cin} />
                            <InfoItem label="Téléphone" value={application.profile?.phone} />
                        </div>
                    </section>
                        ...
                        <div className="space-y-2">
                            {application.documents?.map((doc, idx) => (
                                <button
                                    key={doc.id}
                                    onClick={() => setActiveDocIndex(idx)}
                                    className={`w-full flex items-center justify-between p-3 rounded-xl border-2 transition-all ${idx === activeDocIndex ? 'border-emerald-600 bg-emerald-50 dark:bg-emerald-900/10' : 'border-slate-50 dark:border-slate-800 hover:border-slate-200'}`}
                                >
                                    <div className="flex items-center gap-3">
                                        <div className={`p-2 rounded-lg ${idx === activeDocIndex ? 'bg-emerald-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-400'}`}>
                                            <FileText size={16} />
                                        </div>
                                        <span className={`text-xs font-bold uppercase tracking-tight ${idx === activeDocIndex ? 'text-emerald-700 dark:text-emerald-400' : 'text-slate-600 dark:text-slate-300'}`}>
                                            {docTranslations[doc.document_type] || doc.document_type.replace(/_/g, ' ')}
                                        </span>
                                    </div>
                                    {idx === activeDocIndex && <CheckCircle size={16} className="text-emerald-600" />}
                                </button>
                            ))}
                        </div>
                    </section>
                ...
                <div className="p-6 border-t border-slate-100 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-900/50 space-y-3">
                    ...
                    <div className="flex gap-3">
                        {!hasReceipt && (
                            <button
                                onClick={() => onStatusChange(application.id, 'awaiting_receipt')}
                                className="flex-1 btn btn-outline border-purple-200 text-purple-600 hover:bg-purple-50 dark:hover:bg-purple-900/20 py-3 text-xs uppercase tracking-widest font-black"
                            >
                                Demander Reçu
                            </button>
                        )}
                        <button
                            ...
                            className="flex-1 btn btn-outline border-pink-200 text-pink-600 hover:bg-pink-50 dark:hover:bg-pink-900/20 py-3 text-xs uppercase tracking-widest font-black"
                        >
                            Demander Correction
                        </button>
                    </div>
